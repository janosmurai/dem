...
def open_dev_env_settings_panel(all_tool_images: dict[str, ToolImage]) -> list[str]:
    ...
    if dev_env_settings_window.button_pressed is None or \
        dev_env_settings_window.button_pressed is dev_env_settings_window.cancel_button_id:
        raise typer.Abort()
...
def execute(platform: Platform, dev_env_name: str) -> None:
    """ Create a new Development Environment.
        ...
        Exceptions:
            Abort -- if the name of the Development Environment contains whitespace characters
    """
    platform.get_tool_image_info_from_registries = False
